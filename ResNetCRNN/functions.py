...
class Dataset_CRNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform, frame_nums=64):
        # X = []
        # for i in self.frames:
        ...
        X = []
        while True:
            cur_mp4_alljpgs = sorted([fn for fn in os.listdir(os.path.join(path, selected_folder)) if fn.endswith(".png")], key=exact_num)
            interval = random.choice([1])
            sta = random.choice(range(0, len(cur_mp4_alljpgs)))
            for img_idx in range(sta, len(cur_mp4_alljpgs), interval):
                img = cur_mp4_alljpgs[img_idx]
                
                if img.endswith(".png"):
                    image = Image.open(os.path.join(path, selected_folder, img))
                else:
                    continue
                if use_transform is not None:
                    image = use_transform(image)
    ...
    def __getitem__(self, index):
        ...
        # Load data
        X = self.read_images(self.data_path, folder, self.transform, frame_nums=64)     # (input) spatial images
        y = torch.LongTensor([self.labels[index]])                  # (labels) LongTensor are for int64 instead of FloatTensor
...
class ResCNNEncoder(nn.Module):
    def __init__(self, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300):
        ...
        custom_weight_path = "/home/yjc/.cache/torch/hub/checkpoints/resnet152-394f9c45.pth"
        resnet = models.resnet152(pretrained=False)
        try:
            # 从自定义路径加载权重文件
            state_dict = torch.load(custom_weight_path)
            # 将加载的权重应用到模型上
            resnet.load_state_dict(state_dict)
            print("成功从自定义路径加载模型权重。")
        except Exception as e:
            print(f"加载权重时出现运行时错误：{e}")
